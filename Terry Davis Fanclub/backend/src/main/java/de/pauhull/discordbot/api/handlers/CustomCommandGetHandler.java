...
public class CustomCommandGetHandler implements RequestHandler {
    ...
    public String handleRequest(String[] path, Session session, HttpExchange httpExchange, Api api) {
        ...
        return api.getGson().toJson(DiscordBot.getInstance().getCustomCommandManager().getCustomCommands());
    }
}
